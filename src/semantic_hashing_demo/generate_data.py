...
class ParagraphData(BaseModel):
    ...
    very_similar:str = Field(description="A paragraph that is almost identical to the original paragraph with only a couple of words changed")
...
def generate_data():
    ...
    new_data = marvin.generate(n=2, target=ParagraphData, instructions="generate paragraphs for comparison testing. the paragraphs should be almost identical, with only a few words changed. each paragraph should be at least 100 words long.")
    return new_data
...
with ThreadPoolExecutor(max_workers=num_parallel_calls) as executor:
    ...
    future_to_generate = {executor.submit(generate_data) for _ in range(num_parallel_calls)}
